...
impl<S: Num +  Debug + Display + Clone + Copy + PartialEq> TimeSignal<S> for StepFunction<S> {
...
impl<S:  Num +  Debug + Display + Clone + Copy + PartialEq> fmt::Display for StepFunction<S> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "Step(step_time={}, pre={}, post={}", self.step_time, self.pre_value, self.post_value)
    }
}
